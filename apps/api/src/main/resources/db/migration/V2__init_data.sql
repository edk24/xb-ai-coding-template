...
VALUES
  ...
  (24, 2, '定时任务', 'menu', '/system/scheduled-jobs', 'system/scheduled-jobs', 'scheduled-jobs:view', 'ClockCircleOutlined', 6, 0, 1, ''),
  (25, 24, '新增任务', 'button', '', '', 'scheduled-jobs:create', '', 1, 0, 1, ''),
  (26, 24, '编辑任务', 'button', '', '', 'scheduled-jobs:update', '', 2, 0, 1, ''),
  (27, 24, '删除任务', 'button', '', '', 'scheduled-jobs:delete', '', 3, 0, 1, ''),
  (28, 24, '手动执行任务', 'button', '', '', 'scheduled-jobs:run', '', 4, 0, 1, '')
ON DUPLICATE KEY UPDATE name = VALUES(name), icon = VALUES(icon), route_path = VALUES(route_path), component_path = VALUES(component_path);
...
INSERT INTO scheduled_jobs (id, name, code, job_key, cron_expression, params, status, allow_manual, lock_at_most_seconds, remark)
VALUES (
  1,
  '系统示例任务',
  'system_noop',
  ...
  1,
  600,
  '内置示例任务，默认停用，可用于验证手动执行和日志'
)
ON DUPLICATE KEY UPDATE name = VALUES(name), job_key = VALUES(job_key), allow_manual = VALUES(allow_manual), remark = VALUES(remark);
